...
MixturePTA::GetAdsorptionPotentialInvoker(std::vector<std::vector<double>> potential_params, std::vector<Fluid> fluids)
{
	...
	else if (this->potential == "LEE")
	{
		return [=](std::size_t i, double z)
		{
			return LEE(z, potential_params[i][0], fluids[i].LennardJonnesDiameter, this->adsorbent);
		};
	}
	else
	{
...
MixturePTA::get_mixture_lj_loading(std::vector<double> bulk_composition, double bulk_pressure, double Temperature, std::vector<std::vector<double>> potential_parameters, std::vector<Fluid> fluids)
{
	...
	call_potential_mix get_potential = GetAdsorptionPotentialInvoker(potential_parameters, fluids);
	...
	for (j = 0; j < ncomp; j++)
	{
		adsorbed_loading[j] = -1.0 * potential_parameters[j][2] * integrate(mix_integral[j], deltas[j]) * 1.0e-7;
	}
