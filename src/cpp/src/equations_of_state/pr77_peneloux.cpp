 ...
 */
std::tuple<double, double> pr77_peneloux::get_critical_properties(double T, double Pc, double Tc, double w)
{
    double Tr = T / Tc;
 ...
 */
std::tuple<double, double, double, double> pr77_peneloux::get_gibbs_energy(double P, std::vector<double> Z, double A, double B, int min_or_max)
{
    double phi, fug;
    if (min_or_max == 0)
    {
...
struct mono_eos pr77_peneloux::get_mono_fluid_properties(double P, double T, Fluid fluid)
{
    ...
    vol = CalculatePurePenelouxVolumeTranslation(vol, fluid);
    double dens = 1.0 / vol;
...
struct mix_eos pr77_peneloux::get_mixture_fluid_properties(std::vector<double> x, double P, double T, std::vector<Fluid> fluids)
{
    ...
    vol = CalculateMixturePenelouxVolumeTranslation(vol, x, fluids);
