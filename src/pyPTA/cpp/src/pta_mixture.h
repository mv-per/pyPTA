...
class MixturePTA : public BasePTA
{
...
public:
	...
	std::vector<double> get_loading(std::vector<double> composition, double P, double T, std::vector<std::vector<double>> potential_params, std::vector<Fluid> fluids)
	{
		call_mix_get_load _get_loading = this->get_proper_loader();
		return _get_loading(composition, P, T, potential_params, fluids);
	}

private:
	std::function<mix_eos(std::vector<double>, double, double)> get_equation_of_state_mixture(std::vector<Fluid> fluids)
	{
	...
	std::function<std::vector<double>(std::vector<double>, double, double, std::vector<std::vector<double>>, std::vector<Fluid>)> get_proper_loader(void)
	{
	...
	get_adsorption_potential(std::vector<std::vector<double>> potential_params, std::vector<Fluid> fluids)
	{
		if (this->potential == "DRA")
		{
		...
		else if (this->potential == "STELLE")
		{
			return [=](std::size_t i, double z)
			{
				return STEELE(z, potential_params[i][0], fluids[i].lj_diameter, this->adsorbent_properties);
			};
		}
		else if (this->potential == "LEE")
		{
			return [=](std::size_t i, double z)
			{
				return LEE(z, potential_params[i][0], fluids[i].lj_diameter, this->adsorbent_properties);
			};
		}
		else
		{
	...
	get_mixture_lj_loading(std::vector<double> bulk_composition, double bulk_pressure, double Temperature, std::vector<std::vector<double>> potential_parameters, std::vector<Fluid> fluids)
	{
		...
		size_t ncomp = bulk_composition.size();

		call_potential_mix get_potential = get_adsorption_potential(potential_parameters, fluids);
		...
		for (j = 0; j < ncomp; j++)
		{
			minimal_space = this->min_int * fluids[j].lj_diameter;
		...
		for (i = 0; i < this->num_of_layers; i++)
		{
			for (j = 0; j < ncomp; j++)
			{
				eps = get_potential(j, z[j][i]) + get_potential(j, potential_parameters[j][2] - z[j][i]);
				f_eps[j] = get_f_eps(eps, Temperature);
			}
			...
			for (j = 0; j < ncomp; j++)
			{
				mix_integral[j][i] = this->get_density_value(adsorbed.dens * adsorbed_composition[j], bulk.dens * bulk_composition[j]);
			}
		}

		for (j = 0; j < ncomp; j++)
		{
			adsorbed_loading[j] = -1.0 * potential_parameters[j][2] * this->integrate(mix_integral[j], deltas[j]) * 1.0e-7;
		}
	...
	get_mixture_dr_loading(std::vector<double> bulk_composition, double bulk_pressure, double Temperature, std::vector<std::vector<double>> potential_parameters, std::vector<Fluid> fluids)
	{
		...
		for (std::size_t i = 0; i < this->num_of_layers; i++)
		{
			for (j = 0; j < ncomp; j++)
			{
				eps = get_potential(j, z[j][i]);
				f_eps[j] = get_f_eps(eps, Temperature);
			}
		...
		for (j = 0; j < ncomp; j++)
		{
			adsorbed_loading[j] = this->integrate(mix_integral[j], deltas[j]) / 1000.0;
		}
